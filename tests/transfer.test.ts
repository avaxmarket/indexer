...
const test_main = async (
    start_block_number: number, end_block_number: number,
) => {
                        ...
                        if (
                            ...
                            for await (const input of inscription.vin) {
                                ...
                                if (utxo?.owner?.toLocaleLowerCase() !== from) {
                                    ...
                                    console.log('utxo?.owner?.toLocaleLowerCase() !== from', utxo?.owner?.toLocaleLowerCase(), from)
                                }
                            ...
                            if (input_value.isLessThan(output_value)) {
                                isTransfer = false
                                console.log('input_value isLessThan output_value', input_value.dividedBy(DEC).toString(), output_value.dividedBy(DEC).toString())
                            }
...
const test_next_transfer_main = async () => {
    let block_number = 4931492
    await test_main(block_number, block_number + PRE_BLOCK_NUMBER)
}
test_next_transfer_main()